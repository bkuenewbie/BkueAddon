...
@AbilityManifest(name = "바운드 점퍼", rank = Rank.A, species = Species.HUMAN, explain = {
        "§7철괴 우클릭 §8- §b버티컬 도약§f: 바라보는 방향 상공으로 강하게 회전 점프합니다.",
        "§7패시브 §8- §e에어 바운드§f: 공중에 떠 있는 상태에서 웅크리기(Shift)를 누르면",
        "- 공중에서 한 번 더 고도 도약을 실행합니다. (도약마다 개별 쿨타임 적용)",
...
public class BoundJumper extends AbilityBase implements ActiveHandler {
    ...
    private final Cooldown activeCooldown = new Cooldown(ACTIVE_COOLDOWN.getValue(), "버티컬 도약");
    private final Cooldown passiveCooldown = new Cooldown(PASSIVE_COOLDOWN.getValue(), "에어 바운드");
    ...
    public void onPlayerSneak(PlayerToggleSneakEvent e) {
        if (e.getPlayer().equals(getPlayer()) && e.isSneaking()) {
            if (((org.bukkit.entity.Entity) getPlayer()).isOnGround()) return;
