...
@AbilityManifest(name = "인챈트 부자", rank = Rank.L, species = Species.HUMAN, explain = {
        "§7패시브 §8- §e황금의 손길§f: 게임 시작 시 가지고 있는 아이템을 포함하여, 인벤토리에 들어오는 모든 장비와 무기의 인챈트를 제거하고 최고의 인챈트를 부여합니다.",
        "- §b[보호 IV / 내구성 III / 날카로움 V / 발화 II]",
        "§7아이디어 제공 §8- §6goodhyojun"
}, summarize = {
        "장비와 무기의 인챈트를 제거하고 최고의 인챈트를 부여합니다."
})
public class EnchantRich extends AbilityBase {
    ...
    private boolean applyEnchants(ItemStack item) {
        ...
        boolean equipment = type.name().endsWith("_HELMET")
                || type.name().endsWith("_CHESTPLATE")
                || type.name().endsWith("_LEGGINGS")
                || type.name().endsWith("_BOOTS");

        boolean weapon = type.name().endsWith("_SWORD")
                || type.name().endsWith("_AXE");

        if (!equipment && !weapon && type.getMaxDurability() <= 0) return false;
    ...
    protected void onUpdate(Update update) {
        if (update == Update.RESTRICTION_CLEAR) {
            ...
            ParticleLib.ENCHANTMENT_TABLE.spawnParticle(
                    getPlayer().getLocation().add(0, 1, 0),
                    0.5f, 0.8f, 0.5f, 20, 0.1
            );
