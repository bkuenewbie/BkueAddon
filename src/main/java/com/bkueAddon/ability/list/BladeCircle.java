    ...
    private final Duration duration = new Duration(100) {
        ...
        protected void onDurationProcess(int count) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                ParticleLib.SWEEP_ATTACK.spawnParticle(center.clone().add(Math.cos(angle) * 3.5, 0, Math.sin(angle) * 3.5), 0, 0, 0, 1, 0);
            }
            ...
            for (Entity entity : getPlayer().getWorld().getNearbyEntities(loc, 3.5, 3, 3.5)) {
                if (entity instanceof LivingEntity && !entity.equals(getPlayer()) && loc.distanceSquared(entity.getLocation()) <= 12.25) {
